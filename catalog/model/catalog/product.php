...
//                }
        ...
        //фильтры для gemstones
        if (!empty($data['gemston_filter_stone_type'])) {
            $sql .= " AND p.filtr_stone_type IN (".$data['gemston_filter_stone_type'].")";
        }

        if (!empty($data['gemston_filter_shape'])) {
            $sql .= " AND p.filtr_shape IN (".$data['gemston_filter_shape'].")";
        }

        if (!empty($data['gemston_primary_color'])) {
            $sql .= " AND p.filtr_primary_color IN (".$data['gemston_primary_color'].")";
        }
        ...
        if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
            ...
            if (!empty($data['filter_name'])) {
                $sql .= " OR p.sku LIKE '%".$data['filter_name']."%'";
            }
...
//			}
    ...
    public function getProductMetal ($product_id){
        $product_data = array();
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_metal pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.metal_product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

        foreach ($query->rows as $result) {
            $product_data[$result['metal']] = array('href' => $this->url->link('product/product', 'product_id=' . $result['product_id']), 'name' => list_metal($result['metal']));
        }
